...
class getOwnerResponseModel {
  getOwnerResponseModel({
    this.success,
    this.message,
    this.data,
  });
  ...
  factory getOwnerResponseModel.fromJson(Map<String, dynamic> json) =>
      getOwnerResponseModel(
...
class getOwnerViewModel {
  Future<List<dynamic>> getOwnerName({required BuildContext context}) async {
    late List<dynamic> responseObj;

    try {
      await client
          .request(ServiceApi.getOwner, data: {}, isAuthenticated: false)
          .then(
            (response) async {
